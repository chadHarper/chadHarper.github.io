...
def count_words(text)
  ...
  [words.count { |word| word.casecmp?("the") }, words.length]
end
...
blurbs = Dir[File.join(ROOT, "_papers", "*.md")].map do |path|
  ...
  body = source.sub(/\A---.*?---\s*/m, "").sub(/\n## (?:Further reading|Paper).*?\z/m, "")
  [order, "#{status} guide", title, *count_words(body)]
...
landing.scan(/<article class="research-entry research-entry--(manuscript|question)"[^>]*>(.*?)<\/article>/m) do |kind, body|
  ...
  blurbs << [kind.capitalize, title, *count_words(copy)]
end
